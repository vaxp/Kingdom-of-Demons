...
if [ "$STORE_PROVIDER" == "none" ]; then
    ...
    print_ok "Adding new app called VAXP-OS Software..."
    cat << EOF > /usr/share/applications/VAXP-OS-software.desktop
[Desktop Entry]
Name=Apps Store
...
Comment[zh_CN]=浏览 VAXP-OS 的软件商店并安装我们验证过的应用
Comment[zh_TW]=瀏覽 VAXP-OS 的軟體商店並安裝我們驗證過的應用
Comment[zh_HK]=瀏覽 VAXP-OS 的軟體商店並安裝我們驗證過的應用
Comment[ja_JP]=VAXP-OS のソフトウェアコレクションを閲覧し、検証済みのアプリケーションをインストールします
Comment[ko_KR]=VAXP-OS 소프트웨어 컬렉션을 탐색하고 검증된 애플리케이션을 설치합니다
Comment[vi_VN]=Duyệt bộ sưu tập phần mềm của VAXP-OS và cài đặt các ứng dụng đã được xác minh của chúng tôi
Comment[th_TH]=เรียกดูคอลเลกชันซอฟต์แวร์ของ VAXP-OS และติดตั้งแอปพลิเคชันที่ได้รับการตรวจสอบของเรา
Comment[de_DE]=Durchsuchen Sie die Softwarekollektion von VAXP-OS und installieren Sie unsere verifizierten Anwendungen
Comment[fr_FR]=Parcourez la collection de logiciels d'VAXP-OS et installez nos applications vérifiées
Comment[es_ES]=Explore la colección de software de VAXP-OS e instale nuestras aplicaciones verificadas
Comment[ru_RU]=Просматривайте коллекцию программного обеспечения VAXP-OS и устанавливайте наши проверенные приложения
Comment[it_IT]=Esplora la collezione di software di VAXP-OS e installa le nostre applicazioni verificate
Comment[pt_PT]=Explore a coleção de software da VAXP-OS e instale nossos aplicativos verificados
Comment[pt_BR]=Explore a coleção de software da VAXP-OS e instale nossos aplicativos verificados
Comment[ar_SA]=تصفح مجموعة البرامج الخاصة بـ VAXP-OS وقم بتثبيت تطبيقاتنا الموثقة
Comment[nl_NL]=Blader door de softwarecollectie van VAXP-OS en installeer onze geverifieerde applicaties
Comment[sv_SE]=Bläddra i VAXP-OS programvarusamling och installera våra verifierade applikationer
Comment[pl_PL]=Przeglądaj kolekcję oprogramowania VAXP-OS i instaluj nasze zweryfikowane aplikacje
Comment[tr_TR]=VAXP-OS'un yazılım koleksiyonunu göz atın ve doğrulanmış uygulamalarımızı yükleyin
Categories=System;
