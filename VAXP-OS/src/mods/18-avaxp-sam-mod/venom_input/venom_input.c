...
void apply_keyboard_settings(const char *layouts) {
    char command[512];
    // نستخدم القواعد التي اتفقنا عليها (evdev + pc105 + alt_shift toggle)
    snprintf(command, sizeof(command), 
        "setxkbmap -rules evdev -model pc105 -layout '%s' -option grp:alt_shift_toggle,grp:win_space_toggle", 
        layouts);
...
void save_config(const char *layouts) {
    ...
    if (f == NULL) {
        printf("❌ Error: Cannot write to %s (Run as root?)\n", CONFIG_FILE);
        return;
    }
...
void load_config() {
    ...
    if (f == NULL) {
        printf("⚠️  No config file found. Using default (us).\n");
...
static void on_bus_acquired(GDBusConnection *connection, const gchar *name, gpointer user_data) {
    ...
    printf("🚀 Venom Input Daemon Started.\n");
