...
if [[ $DEFAULT_APPS =~ "calculator" ]]; then
    DESKTOP_FILE="/usr/share/applications/calculator.desktop"
    print_ok "Patching calculator localization..."
    ...
    for locale in "${!LOCALIZED_NAMES[@]}"; do
        ...
        judge "Patch calculator localization for $locale"
    done
