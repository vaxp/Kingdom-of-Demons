...
static void mpris_call(const gchar *method) {
    ...
    if (!player) {
        printf(">> [MPRIS] No active media player found\n");
        return;
    }
...
int XVenomErrorHandler(Display *d, XErrorEvent *e) {
    if (e->error_code == BadAccess && e->request_code == X_GrabKey) {
        printf(">> [Warning] Key already grabbed by another app\n");
        return 0;
    }
    char buffer[1024];
    XGetErrorText(d, e->error_code, buffer, 1024);
    printf(">> [X Error] Code: %d (%s)\n", e->error_code, buffer);
    return 0;
}
...
void load_config(Display *display) {
    ...
    if (!file) {
        printf(">> [Config] /etc/venom/shortcuts.csv not found\n");
        return;
    }
    ...
    fclose(file);
    printf(">> [Config] Loaded %d shortcuts\n", shortcut_count);
}
...
int main() {
    printf("🚀 ════════════════════════════════════════════════════════════\n");
    printf("🚀 Venom Shortcuts Daemon v5.0 (MPRIS Edition)\n");
    printf("🚀 ════════════════════════════════════════════════════════════\n");
    ...
    if (error) {
        printf(">> [D-Bus] Failed to connect: %s\n", error->message);
        g_error_free(error);
    } else {
        printf(">> [D-Bus] Connected to session bus\n");
    }
    
    // Initialize X11
    Display *display = XOpenDisplay(NULL);
    if (!display) {
        printf(">> [X11] Failed to open display\n");
        return 1;
    }
    ...
    XSync(display, False);
    printf("✅ Ready. Listening for shortcuts...\n\n");
